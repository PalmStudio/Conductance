...
```{r}
VPD_df= fread("data/VPDdatabasedPLPE2018_R.csv", data.table = FALSE, fill = TRUE)
# CAREFULL !!!!
...
VPD_df%>%
  ggplot(aes(y= gs, x= VpdL))+
  geom_point(aes(color=Progeny))+
  facet_grid(~Season)+
  ylab("Gs (umol m-2 s-1)")+
  xlab("VPD (kPa)")+
  ggtitle("Effect of VPD on stomatal conductance according to the season and Progeny")

ggsave(filename = "output/Gs_vs_VPD.png", width = 18,height = 12, units = "cm")
```
